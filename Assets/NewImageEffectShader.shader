...
#define _Size 0.3 //size of BH
		...
		vec4 background(vec3 ray)
		{
			...
			vec4 nebulae = tex2D(iChannel0, (uv*1.5));
		...
		vec4 raymarchDisk(vec3 ray, vec3 zeroPos)
		{
			...
			for (float i = 0.; i < _Steps; i++)
			{
				...
				o = clamp(vec4(col*alpha + o.rgb*(1. - alpha), o.a*(1. - alpha) + alpha), vec4(0., 0., 0., 0.), vec4(1., 1., 1., 1.));
		...
		void Rotate(inout vec3 vec3, vec2 angle)
		{
			vec3.yz = cos(angle.y)*vec3.yz
				+ sin(angle.y)*vec2(-1, 1)*vec3.zy;
			vec3.xz = cos(angle.x)*vec3.xz
				+ sin(angle.x)*vec2(-1, 1)*vec3.zx;
		}

		//vec4  mainImage(out vec4 colOut, in vec2 fragCoord)
		vec4  mainImage(in vec2 fragCoord)
		{
			...
			for (int j = 0; j < AA; j++)
				for (int i = 0; i < AA; i++)
				{
					...
					float brightness = -(20.*iMouse.xy / iResolution.y - 10.)*(20.*iMouse.xy / iResolution.y - 10.)*0.05;
					vec3 pos = vec3(0., 0.05, brightness);
					...
					vec4 outCol = vec4(100., 100., 100., 100.);
					...
					col = outCol;
					col.rgb = pow(col.rgb, vec3(.6, .6, .6));
